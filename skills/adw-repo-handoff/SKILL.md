...
4. Fetch required evidence through supported read tools. Reuse an already available sufficient tool schema; discover it only when missing or insufficient, and do not rediscover it in the same context unless capability/version evidence changed. Use focused excerpts plus their complete dependencies; do not dump unrelated history. Reuse already-inspected immutable content only when identity and coverage remain valid. Never substitute a search snippet or producer narrative for required evidence.
